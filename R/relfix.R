#' Provides an age structure to relationship status, when a relationship status has been previously defined using age groups.
#' Redistributes a user-defined relationship status value between ages, using age groups and other variables (if specified). Within the group definition provided, the marginal total of that relationship status is retained. 
#' The data frame can include groups where all people have the relationship status. In this situation, there is no need to restrict the data frame to only those whose relationship status must be redistributed.
#' @export
#' @param people A data frame containing all people with a relationship status.
#' @param pplid The variable that contains the unique identifier for each person, in the people data frame.
