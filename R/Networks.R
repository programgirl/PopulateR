...
#' @param HouseholdCol The column number for each person's household.
#' @param NetworkCol The column number for the network size.
#' @param SDUsed The standard deviation for the age differences between two people on an edge.
#' @param ProbSameNetwork The probability that a friend of a friend is an edge. For example, if A and B and friends, and B and C are friends, this is the probability that C is also a friend of A.
...
SocialNetworks <- function(People, IDCol=NULL, AgeCol=NULL, HouseholdCol=NULL, NetworkCol=NULL,
                           SDUsed=NULL, ProbSameNetwork = .5, #NetworkVariable = NULL,
                           UserSeed=NULL,
                           #pValueToStop=NULL,
  ...
  #####################################
    ...
    WorkingDataFrame <- People %>%
      rename(ID = !! IDCol, Age = !! AgeCol, Household = !! HouseholdCol,
             Network = !! NetworkCol) %>%
      select(ID, Age, Household, Network)

    # check that we don't end up with one person needing a match and none to spare

    if (!(sum(WorkingDataFrame) %% 2 == 0) == TRUE){
      stop("The number of network links must be a factor of 2.")
    }
  ...
  current_clustering <- transitivity(WiredNetwork)
