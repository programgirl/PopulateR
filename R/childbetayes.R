...
#' This function creates a data frame of child-parent/guardian pairs, based on a four-parameter beta distribution of age differences. In addition to the a and b of the beta distribution, location and scale values are required. The location and scale modify the distribution range so that it no longer lies between 0 and 1.
#' Two data frames are required: one for children and one for potential parents.
#' The minimum and maximum ages of parents must be specified. This ensures that there are no parents who were too young (e.g. 11 years) or too old (e.g. 80 years) at the time the child was born. The presence of too young and too old parents is tested throughout this function. Thus, pre-cleaning the parents data frame is not required.
#' This function uses the pre-existing household identifier for the parent. If the parent does not have a household identifier, use the childbetano function instead.
#'
#' @export
...
#' @param a The a shape parameter for the beta distribution.
#' @param b The b shape parameter for the beta distribution.
#' @param location The minimum value for the shifted distribution.
#' @param scale The maximum value for the shifted distribution.
...
childyes <- function(children, chlidcol, chlagecol, parents, paridcol, paragecol, a, b, location, scale,
...

{
  ...
  if (!any(duplicated(parents[paridcol])) == FALSE) {
    stop("The column number for the ID variable in the parents data frame must be supplied, and the ID must be unique to each parent.")
  }
  ...
  if(is.null(hhidcol)) {
    stop("The column number for the household identifier in the parents data frame must be supplied.")
  }
  ...
  for (j in 1:nrow(childrenRenamed)) {
    ...
    Interim <- rbeta(1, a, b, log = FALSE) 
    AgeDifference <- round(location + ((scale-location)*Interim), 0)
    
    cat("Age difference is", AgeDifference, "\n")
  ...
  # only do the next loop if there are children that remain to be matched
 ...
 # cat("The number of children that need to be matched in future is", nrow(childrenRenamed), "\n")
  ...
  for (j in 1:nrow(childrenRenamed)) {
   ...
   # cat("Current child is", Currentchild$ChildID, "Current mins is", Currentmin, "Currentmax is", Currentmax, "\n")
    ...
    # this is the second if(nrow(childrenRenamed) > 0) test
  }
  
  # closes first if(nrow(childrenRenamed) > 0) test
  }
