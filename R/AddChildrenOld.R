...

{
  ...
  if (TwinRate > 0) {
    ...
    for (c in 1:nrow(TwinsMatched)) {
      ...
      # cat("1 age_index = ", age_index, "Age diff is", TwinsMatched$AgeDifference[c], "\n")
    ...
    if (NumChildren > 2) {
      ...
      for (x in 1:nrow(TwinsMatched)) {
        ...
        for (y in (ncol(TwinsMatched) - (NumChildren - 3)):ncol(TwinsMatched)) {
          ...
          cat("2 age_index = ", age_index, "length of ChildrenAgeCountVector[age_index] = ", length(ChildrenAgeCountVector[age_index]), "Age diff is", AgeDifference, "\n")
