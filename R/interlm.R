...
interlm <- function(people, grpdef = NULL, pplagecol = NULL, pplpropcol = NULL, intmin = NULL, intmax = NULL, 
                    ordersize = 3, userseed = NULL) {
  ...
  AgeRange <- data.frame(OriginalAge = seq(from = intmin, to = intmax))
  ...
  for(i in 1:nrow(PeopleUnique)) {
    ...
    CurrentModel <- lm(OriginalProp ~ poly(OriginalAge, ordersize), data = CurrentGroup)
    
    cat("The current group is '", GroupInfo, "' and the imputed model is", "\n")
    
    print(CurrentModel)
    
    CurrentEstimates <- predict(CurrentModel, newdata = AgeRange, se.fit = TRUE)
    AgeRange$Fits <- CurrentEstimates$fit

    # create expansion of grouping
    ExpandedGroup <- CurrentDef %>%
      slice_sample(n = nrow(AgeRange), replace = TRUE)

    WithGroupInfo <- bind_cols(AgeRange, ExpandedGroup) 
