...
#' This function creates a data frame of child-parent/guardian pairs, based on a log-normal distribution of age differences. The log-normal function called uses meanlog and sdlog as the parameters, so these are input directly into this function.
...
#' @param meansd the standard deviation of the distribution on the log scale.
...
childyes <- function(children, chlidcol, chlagecol, parents, paridcol, paragecol, meanlog, sdlog,
...

{
  ...
  for (j in 1:nrow(childrenRenamed)) {
    ...
    AgeDifference <- round(rlnorm(1, meanlog, sdlong),0)
