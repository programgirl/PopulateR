                      ...
                      UserSeed=NULL)
{
  ...
  if (!(nrow(TooManyKids)==0)) {
    ...
    stop(paste("The number of children at these ages exceeds the available school roll places: ", TooManyKids))
  ...
  for(i in 1:nrow(ListofHouseholds)) {
    ...
    while(NumKidsRemaining > 0) {
      ...
      if(nrow(ChildSchoolMerge) > MaxchildrenCanTake) {
        ...
        cat("nrow(ChildSchoolMerge is", nrow(ChildSchoolMerge), " MaxchildrenCanTake is",  MaxchildrenCanTake, "\n")
