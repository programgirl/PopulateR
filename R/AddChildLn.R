...
# MinMotherAge = the youngest age at which a woman gives birth
# MaxMotherAge = the oldest age at which a woman gives birth
...
# UserSeed=NULL
# HouseholdNumVariable = The column name for the household variable. This must be supplied in quotes.
# pValueToStop = The primary stopping rule for the function. If this value is not set, the critical p-value of .01 is used.

GetMotherAges <- function(ChildDataframe, ChildIDVariable, ChildAgeVariable, meanlogUsed, sdlogUsed, MotherDataframe, MotherIDVariable,
                          MotherAgeVariable, MinMotherAge, MaxMotherAge, PropMothers, MinPropRemain,UserSeed=NULL, HouseholdNumVariable,
                          pValueToStop = ){
  ...
  options(dplyr.summarise.inform=F)
   ...
   # for mothers
    ...
    # Uses midpoint rule.
    logProbLow <- dlnorm(-MaxMotherAge:(min_bin-0.5), meanlog = meanlogUsed, sdlog = sdlogUsed, log=TRUE)
  ...
  #   MothersMatched <- left_join(MatchedMotherAges, rename_at(MothersToMatch, MotherAgeVariable, ~ names(MatchedMotherAges)[1]), by = c(names(MatchedMotherAges)[1], "MotherAgeCount")) %>%
